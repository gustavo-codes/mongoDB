...
async def criar_obra(obra:ObraBase):
    ...
    construcao = await construcao_collection.find_one({'_id':ObjectId(obra.contrucao_id)})
    if not construcao:
        raise HTTPException(detail='Contrução não existe',status_code=404)
    id = await criar('obra',obra)
    await construcao_collection.update_one({'_id':ObjectId(obra.contrucao_id)},{'$addToSet':{'obras_id':ObjectId(id)}})
    return {'msg':'Done'}
...
async def deletar_obra(obra_id:str):
    ...
    await construcao_collection.update_one({'obras_ids':ObjectId(obra_id)},{'$pull':{'obras_ids':ObjectId(obra_id)}})
