...
async def quantidade_total_de_usuarios():
    ...
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Erro ao acessar o total de ocorrencias de usuário. Erro: {e}",
        )
...
async def adicionar_terreno_a_pessoa(pessoa_id: str, terreno_id: str):
    ...
    try:
        ...
        logging.warning(
            f"Problema ao processar a busca por terreno de id {terreno_id}. Erro: {e}"
        )
        raise HTTPException(
            status_code=500,
            detail=f"Problema ao processar a busca por terreno de id {terreno_id}. Erro: {e}",
        )
    ...
    try:
        ...
        logging.warning(
            f"Problema ao processar busca por pessoa de id {pessoa_id}. Erro: {e}"
        )
        raise HTTPException(
            status_code=500,
            detail=f"Problema ao processar busca por pessoa de id {pessoa_id}. Erro: {e}",
        )
